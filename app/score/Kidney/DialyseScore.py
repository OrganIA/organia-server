...
def getDate(receiver_listing: Listing, listing_kidney):
    if listing_kidney.isDialyse is False:
        ...
        listing_kidney.EndDateDialyse is not None
        and listing_kidney.EndDateDialyse > listing_kidney.DateTransplantation
    ):
...
def getScore(receiver_listing: Listing, listing_kidney):
    try:
        s = (
            ...
            - getDate(receiver_listing, listing_kidney)
        ).days
        if s > 3650:
            return 1
        elif s < 0:
...
def getWaitingTime(receiver_listing: Listing, listing_kidney):
    ...
    if listing_kidney.isRetransplantation or (DATT - DDIAL).days < 365:
        ...
        listing_kidney.isRetransplantation is False
        and (listing_kidney.startDateDialyse - listing_kidney.startDateDialyse)
        >= 365
    ):
        return 12 + DDIAL
