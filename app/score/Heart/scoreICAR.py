...
def getF_Decile_PNj(CEC, CAT, SIAV, DBNB, BNP, PROBNP, Date_Courante, DPROBNB, Delai_Var_Bio_LA):
   if CEC == 'O'or CAT == 'O' or SIAV == 'B':
       ...
       return 1
   elif PROBNP != None and Date_Courante - DPROBNB <= Delai_Var_Bio_LA:
        ...
        for index, condition in enumerate(conditions):
            if PROBNP < condition:
                return res[index]
   elif BNP != None and Date_Courante - DBNB <= Delai_Var_Bio_LA:
...
def getF_DFGj(SEXR, AGER, CREAT):
    if SEXR == 'F':
        return 186.3 * ((CREAT/88.4)*-1.154) * (AGER*-0.203) * 0.742
    else:
        return 186.3 * ((CREAT/88.4)*-1.154) * (AGER*-0.203) * 1

def getF_Ln_DFG_LAj(DIA2, CREAT, DCREAT, SEXR, AGER, Date_Courante, Delai_Var_Bio_LA):
    F_DFGj = getF_DFGj(CREAT, AGER, SEXR)
    if DIA2 == 'O':
        return ln(15)
    elif CREAT == None or (Date_Courante - DCREAT) > Delai_Var_Bio_LA:
        return ln(150)
    else:
...
def getF_Ln_BILI_LAj(BILI, DBILI, Date_Courante, Delai_Var_Bio_LA):
    if BILI == None or (Date_Courante - DBILI) > Delai_Var_Bio_LA:
        return ln(5)
    else:
...
def getICAR(model):
    ...
    F_Ln_DFG_LAj = getF_Ln_DFG_LAj(model.DIA2, model.CREAT, model.DCREAT, model.sexR, model.ageR, model.Date_Courante, model.Delai_Var_Bio_LA)
